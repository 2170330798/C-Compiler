...
{
    assert(brackets_node->type == NODE_TYPE_BRACKET);
    vector_push(brackets->n_brackets, &brackets_node);
}
...
{
    return array_brackets_caculate_size_from_index(dtype, brackets, 0);
}

int array_total_indexes(struct datatype* dtype)
{
    assert(dtype->flag & DATATYPE_FLAG_IS_ARRAY);
