...
struct scope* scope_new(struct compile_process* process, int flag)
{
    assert(process->scope.root);
    assert(process->scope.current);
...
void* scope_last_enity_at_scope(struct scope* scope)
{
    if (vector_count(scope->entities) == 0)
        return NULL;
...
void* scope_last_entity_from_scope_stop_at(struct scope* scope, struct scope* stop_scope)
{
    if (scope == stop_scope)
        ...
        void* last = scope_last_enity_at_scope(scope);
        if(last)
        {
            return last;
        }
