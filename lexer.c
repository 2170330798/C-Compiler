...
struct token *token_make_number_for_value(unsigned long number)
{
    ...
    if (number_type != NUMBER_TYPE_INT)
    {
        nextc();
    }
...
struct token *token_make_number(const char c)
{
...
static struct token * token_make_string_or_operator()
{
    char op = peekc();
    if (op == '<')
    {
        struct token* last_token = lexer_last_token();
        if (token_is_keywords(last_token, "include"))
        {
...
bool is_keywords(const char* str)
{
    ...
    return S_EQ(str, "int") ||
           ...
           S_EQ(str, "signed") ||
           S_EQ(str, "_Bool") ||    // C99
           S_EQ(str, "_Complex") || // C99
           S_EQ(str, "_Imaginary") || // C99
...
struct token *handle_comment()
{
    ...
    if (c == '/')
    {
        ...
        pushc('/');
        return token_make_string_or_operator();
    }
    return NULL;
}
...
static struct token* token_make_identifier_or_keywords()
{
    struct buffer *buffer = buffer_create();
    char c = peekc();
    ...
    if (is_keywords(buffer_ptr(buffer)))
    {
...
struct token* read_special_token()
{
    ...
    if(isalpha(c) || c == '_')
    {
        return token_make_identifier_or_keywords();
    }
    return NULL;
}
...
struct token *read_next_token()
{
    ...
    {
        NUMEBER_CASE:
            token = token_make_number(c);
        break;
        OPERATOR_CASE_EXCLUDING_DIVISION:
            token = token_make_string_or_operator();
        break;
        SYMBOL_CASE:
...
int lex(struct lex_process *process)
{
    ...
    lex_process = process;
    process->pos.filename = process->compiler->c_file.abs_path;
...
struct lex_process* tokens_build_for_string(struct compile_process* compiler, const char* str)
{
    ...
    if (!lex_process || lex(lex_process) != LEXIT_ANALYSIS_SUCESS)
    {
        return NULL;
    }
    return lex_process;
