...
static void parser_ignore_nl_or_comment(struct token* token)
{
    ...
    printf("parser_ignore_nl_or_comment is running\n");
}

static struct token* token_next()
{
    printf("token_next is running\n");
    struct token* next_token = vector_peek_no_increment(current_process->token_vec);
    if (next_token == NULL) {
        fprintf(stderr, "Error: next_token is NULL!\n");
        return NULL;
    }
...
static void expect_symbol(char c)
{
...
{
    printf("token_peek_next is running\n");
    struct token* next_token = vector_peek_no_increment(current_process->token_vec);
    parser_ignore_nl_or_comment(next_token);
...
void parse_datatype_modifiers(struct datatype* dtype)
{
    ...
    while(token && token->type == TOKEN_TYPE_KEYWORD)
    {
        ...
        {
            dtype->flag |= DATATYPE_FLAG_IS_SIGNED;
        }
        ...
        {
            dtype->flag |= DATATYPE_FLAG_IS_STATIC;
        }
        else if (S_EQ(token->sval, "const"))
        {
            dtype->flag |= DATATYPE_FLAG_IS_CONST;
        }
        else if (S_EQ(token->sval, "extern"))
        {
            dtype->flag |= DATATYPE_FLAG_IS_EXTERN;
        }
        else if (S_EQ(token->sval, "__ignore_typecheck__"))
        {
            dtype->flag |= DATATYPE_FLAG_IGNORE_TYPE_CHECKING;
        }
...
void parser_get_datatype_tokens(struct token** datatype_token, struct token** datatype_secondary_token)
{
    *datatype_token = token_next();
    ...
    {
        *datatype_secondary_token = token_next();
        token_next();
    }
}
...
void parser_datatype_adjust_size_for_secondary(struct datatype* datatype, struct token* datatype_secondary_token)
{
    ...
    datatype->flag |= DATATYPE_FLAG_IS_SECONDARY;
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out)
{
    ...
    else if(S_EQ(datatype_token->sval, "char"))
    {
        ...
        datatype_out->size = DATA_SIZE_BYPE;
    }
    ...
    {
        compiler_error(current_process, "Bug unexpected primitive datatype\n");
    }
...
void parse_datatype_type(struct datatype* datatype)
{
    ...
    if (datatype_is_struct_or_union_for_name(datatype_token->sval))
    {
        ...
        {
            //This structure hasn't name. so we need to handle it.
            datatype_token = parse_build_random_type_name();
            datatype->flag |= DATATYPE_FLAG_STRUCT_UNION_NO_NAME;
        }
    }
...
void parser_ignore_int(struct datatype* dtype)
{
    ...
    if (!token_is_keywords(token_peek_next(), "int"))
    {
        return ;
    }
    if(!parser_is_int_valid_after_datatype(dtype))
    {
        compiler_error(current_process, "You provide a secondary\"int\" type however its not supported with this current abbrevation\n");
    }
...
void make_variable_node(struct datatype* dtype, struct token* name_token, struct node* value_node)
{
    ...
    node_create(&(struct node){.type=NODE_TYPE_VARIABLE, .var.name=name_token->sval, .var.type=*dtype, .var.val=value_node});
}
...
void make_variable_node_and_register(struct history* history, struct datatype* dtype, struct token* name_token, struct node* value_node)
{
     ...
     #warning "Remember to caculate scope offsets and push to the scope"
     node_push(var_node);
}
...
struct array_brackets* parse_array_brackets(struct history* history)
{
    ...
    while(token_next_is_operator("["))
    {
        ...
        struct node* brackets_node = node_pop();
        array_brackets_add(brackets, brackets_node);
    }
    return brackets;
}
...
void parse_variable(struct datatype* dtype, struct token* name_token, struct history* history)
{
    ...
    if(token_next_is_operator("["))
    {
        ...
        dtype->array.size = array_brackets_caculate_szie(dtype, brackets);
...
void parse_varibale_function_or_struct_union(struct history* history)
{
...
void parse_keywords(struct history* history)
{
    struct token* token = token_peek_next();
    if(is_keyword_variable_modifier(token->sval) || keyword_is_datatype(token->sval))
    {
        parse_varibale_function_or_struct_union(history);
        return;
    }
}
...
void parse_keywords_for_global()
{
    parse_keywords(history_begin(0));
